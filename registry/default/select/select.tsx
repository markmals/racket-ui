...
// Classification: B — React Aria (Select / SelectValue / Button / Popover /
// ListBox / ListBoxItem) + compatibility adapters. shadcn's `value`-keyed API is
// preserved: `Select` accepts `value`/`defaultValue`/`onValueChange`,
// `SelectItem` accepts `value` (mapped to React Aria's `id`/`selectedKey`), and
// `SelectValue` accepts a `placeholder`.
